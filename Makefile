...
strace:
	...
	SESSION=$$(python3 src/collector.py --mode strace --parallel | grep "SESSION_PATH" | cut -d ':' -f2-) && \
...
ebpf:
	...
	SESSION=$$(python3 src/collector.py --mode ebpf --parallel | grep "SESSION_PATH" | cut -d ':' -f2-) && \
	echo "Using session: $$SESSION" && \
